...
@app.route("/")
def root():
    logged_in, FirstName, no_of_items = get_login_details()
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        # Show last product added
        cur.execute(
            'SELECT productId, name, price, description, image, stock FROM products ORDER BY productId DESC LIMIT 4 ')
        # Show all items
    ...
    return render_template('home.html', itemData=item_data, loggedIn=logged_in, firstName=FirstName,
...
def remove_from_cart():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        try:
            cur.execute("DELETE FROM kart WHERE UserId = " + str(UserId) + " AND productId = " + str(product_id))
            conn.commit()
